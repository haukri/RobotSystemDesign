...
bool RTDEControlInterface::sendCommandAsync(const RTDE::RobotCommand &cmd)
{
  std::chrono::high_resolution_clock::time_point start_time = std::chrono::high_resolution_clock::now();
...
bool RTDEControlInterface::commandDoneAsync()
{
  try {
    if(getControlScriptState() != UR_CONTROLLER_DONE_WITH_CMD) {
      return false;
    }
    // Command has been received stop sending it.
    sendClearCommand();
